 ...
 * sorting algorithm
 */
void sorting(record_t *record){

	//char *temp = (char *)key_fptr(record);
	emit_intermediate( key_fptr(record), record, ksize_fptr(record) );
 ...
 */
void mapper_srt(map_args_t *args) 
{
...
void sort_op(final_data_t *in, int (*srt)(const void *k1, const void *k2) , void *(*key_ptr)(void *rec), int (*keysize)(void *rec), final_data_t *op_results, int op_num){
	...
	map_reduce_args.map = mapper_srt;
